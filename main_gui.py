...
def clear_client_fields():# Функции очистки полей 
...
def to_dict_client(client_ex):# Функции для сохранения в файл
    return {
...
def show_warning_modal(message):#Функция показа предупреждающего окна 
...
def update_clients_table():# Функции для таблиц 
...
def update_vehicles_table():
    ...
    for vehicle in company.vehicles:
        with dpg.table_row(parent="vehicles_table"):
            checkbox_tag = f"vehicle_checkbox_{vehicle.vehicle_id}" # Привязываем уникальный чекбокс для каждого транспортного средства по его ID
            dpg.add_checkbox(tag=checkbox_tag)# Чекбокс для каждого транспорта
...
def add_new_transport(sender, app_data):
    ...
    update_vehicles_table()
    dpg.set_value("status_bar", "Транспорт успешно добавлен!")
...
with dpg.font_registry():
    with dpg.font("fonts\RobotoMono-VariableFont_wght (1).ttf", 20) as default_font:
...
with dpg.window(tag="about_window", label="О программе", modal=True, show=False, no_resize=True, width=popup_window_width, height=popup_window_height):
    ...
    dpg.add_text("ФИО:Белькевич Полина Алексеевна")
    dpg.add_spacer(height=height_of_medium_spacer)
    dpg.add_button(label="Закрыть", callback=lambda: dpg.configure_item(
